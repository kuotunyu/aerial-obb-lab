...
The maintained demo is the static Browser-native BYOM workbench in `demo/web/`. Supply a trusted,
compatible `.onnx` file through its model picker; the release does not accept `.pt` files in the
browser, download a named model, or perform an implicit export. Verify the model's provenance and
...
Open `http://localhost:8765`, select the local ONNX model and an image, then explicitly press
Detect. The required `images [1,3,1024,1024]` input and `output0 [1,N,7]` output contract is
documented in [`demo/web/README.md`](../demo/web/README.md).
